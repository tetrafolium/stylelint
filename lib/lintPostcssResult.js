...
function lintPostcssResult(stylelintOptions, postcssResult, config) {
  ...
  const postcssRoots = /** @type {import('postcss').Root[]} */ (
      postcssDoc && postcssDoc.constructor.name === 'Document'
          ? postcssDoc.nodes
          : [ postcssDoc ]);
  ...
  const rules = config.rules
                    ? Object.keys(config.rules)
                          .sort(
                              (a, b) => Object.keys(rulesOrder).indexOf(a) -
  ...
  rules.forEach((ruleName) => {
    const ruleFunction =
        rulesOrder[ruleName] || get(config, [ 'pluginFunctions', ruleName ]);

    if (ruleFunction === undefined) {
      performRules.push(
          Promise.all(
              postcssRoots.map(
                  (postcssRoot) => reportUnknownRuleNames(ruleName, postcssRoot,
                                                          postcssResult),
                  ),
              ),
      );
    ...
    postcssResult.stylelint.customMessages[ruleName] =
        get(secondaryOptions, 'message');

    performRules.push(
        Promise.all(
            postcssRoots.map(
                (postcssRoot) => ruleFunction(primaryOption, secondaryOptions, {
                  fix : stylelintOptions.fix &&
                            // Next two conditionals are temporary measures
 ...
 * The autofixer does not yet adhere to stylelint-disable comments, so if there
 * are disabled ranges we can not autofix this document. More info in issue
 * #2643.
 *
 * @param {PostcssResult} postcssResult
 * @returns {boolean}
...
function isFixCompatible({stylelint}) {
  ...
  if (stylelint.disabledRanges.all.length)
    return false;
