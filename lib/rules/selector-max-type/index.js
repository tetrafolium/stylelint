...
const isStandardSyntaxTypeSelector =
    require('../../utils/isStandardSyntaxTypeSelector');
...
const messages = ruleMessages(ruleName, {
  expected : (selector, max) => `Expected "${selector}" to have no more than ${
      max} type ${max === 1 ? 'selector' : 'selectors'}`,
});
  ...
  return (root, result) => {
    const validOptions = validateOptions(
        ...
        ruleName,
        {
          ...
          possible(max) { return typeof max === 'number' && max >= 0; },
        },
        {
          actual : options,
          possible : {
    ...
    function checkSelector(selectorNode, ruleNode) {
      const count = selectorNode.reduce((total, childNode) => {
        ...
        if (childNode.type === 'tag' &&
            !isStandardSyntaxTypeSelector(childNode)) {
          return total;
        }
        ...
        resolvedNestedSelector(selector, ruleNode)
            .forEach((resolvedSelector) => {
              ...
              parseSelector(
                  ...
                  result,
                  ruleNode,
                  (container) => checkSelector(container, ruleNode),
              );
            });
      });
    });
