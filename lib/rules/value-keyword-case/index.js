...
const isCounterIncrementCustomIdentValue =
    require('../../utils/isCounterIncrementCustomIdentValue');
const isCounterResetCustomIdentValue =
    require('../../utils/isCounterResetCustomIdentValue');
...
// Operators are interpreted as "words" by the value parser, so we want to make
// sure to ignore them.
const ignoredCharacters = new Set([ '+', '-', '/', '*', '%' ]);
const gridRowProps = new Set([ 'grid-row', 'grid-row-start', 'grid-row-end' ]);
const gridColumnProps =
    new Set([ 'grid-column', 'grid-column-start', 'grid-column-end' ]);
...
function rule(expectation, options, context) {
  return (root, result) => {
    ...
    root.walkDecls((decl) => {
      ...
      const parsed =
          valueParser(decl.raws.value ? decl.raws.value.raw : decl.value);
      ...
      parsed.walk((node) => {
        ...
        if (node.type === 'function' &&
            (valueLowerCase === 'url' || valueLowerCase === 'var' ||
             valueLowerCase === 'counter' || valueLowerCase === 'counters' ||
             valueLowerCase === 'attr')) {
          return false;
        }
        ...
        if (node.type === 'function' && ignoreFunctions.length > 0 &&
        ...
        if (expectation === 'lower' &&
          ...
          expectedKeyword =
              mapLowercaseKeywordsToCamelCase.get(keywordLowerCase);
