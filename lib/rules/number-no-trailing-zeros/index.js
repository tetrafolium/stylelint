  ...
  return (root, result) => {
    ...
    function check(node, value, getIndex) {
      ...
      valueParser(value).walk((valueNode) => {
        ...
        // match[1] is any numbers between the decimal and our trailing zero,
        // could be empty match[2] is our trailing zero(s)
        if (match === null) {
          return;
        }
        ...
        // trailing zeros after our decimal. in that case we don't need the
        // decimal either so we move our index back by 1.
        const startIndex = match[1].length > 0 ? index : index - 1;

        // our end index is our original index + the length of our trailing
        // zeros
      ...
      if (fixPositions.length) {
        fixPositions.forEach((fixPosition) => {
          ...
          if (node.type === 'atrule') {
            node.params =
                removeTrailingZeros(node.params, startIndex, endIndex);
          } else {
