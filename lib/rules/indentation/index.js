  ...
  return (root, result) => {
    ...
    root.walk((node) => {
      ...
      if ((lastIndexOfNewline !== -1 ||
           (isFirstChild && (!getDocument(parent) ||
                             parent.raws.beforeStart.endsWith('\n')))) &&
        ...
        if (context.fix) {
          ...
          node.raws.before =
              fixIndentation(node.raws.before, expectedOpeningBraceIndentation);
        } else {
          report({
      ...
      // And check `indentClosingBrace` to see if it should be indented an extra
      // level.
      const closingBraceLevel =
          options.indentClosingBrace ? nodeLevel + 1 : nodeLevel;
      ...
      if (hasBlock(node) && after && after.includes('\n') &&
        ...
        if (context.fix) {
          node.raws.after =
              fixIndentation(node.raws.after, expectedClosingBraceIndentation);
        } else {
          report({
    ...
    function indentationLevel(node, level = 0) {
      ...
      if (optionsMatches(options, 'except', 'block') &&
          (node.type === 'rule' || node.type === 'atrule') && hasBlock(node)) {
        calculatedLevel--;
      }
    ...
    function checkAtRuleParams(atRule, ruleLevel) {
      ...
      // @nest and SCSS's @at-root rules should be treated like regular rules,
      // not expected to have their params (selectors) indented
      const paramLevel = optionsMatches(options, 'except', 'param') ||
                                 atRule.name === 'nest' ||
                                 atRule.name === 'at-root'
                             ? ruleLevel
                             : ruleLevel + 1;
    ...
    function checkMultilineBit(source, newlineIndentLevel, node) {
      ...
      // `outsideParens` because function arguments and also non-standard
      // parenthesized stuff like Sass maps are ignored to allow for arbitrary
      // indentation
      ...
      styleSearch(
          ...
          (match, matchCount) => {
            const precedesClosingParenthesis =
                /^[ \t]*\)/.test(source.slice(match.startIndex + 1));

            if (optionsMatches(options, 'ignore', 'inside-parens') &&
            ...
            if (!optionsMatches(options, 'ignore', 'inside-parens') &&
                match.insideParens) {
              // If the first match in is within parentheses, reduce the
              // parenthesis penalty
              if (matchCount === 1)
              ...
              const followsOpeningParenthesis =
                  /\([ \t]*$/.test(source.slice(0, newlineIndex));

              if (followsOpeningParenthesis) {
                parentheticalDepth += 1;
              }

              const followsOpeningBrace =
                  /{[ \t]*$/.test(source.slice(0, newlineIndex));
              ...
              // Past this point, adjustments to parentheticalDepth affect next
              // line
            ...
            // check that the whitespace characters (excluding newlines) before
            // the first non-whitespace character equal the expected indentation
            const afterNewlineSpaceMatches =
                /^([ \t]*)\S/.exec(source.slice(match.startIndex + 1));
            ...
            if (afterNewlineSpace !== expectedIndentation) {
              if (context.fix) {
                // Adding fixes position in reverse order, because if we change
                // indent in the beginning of the string it will break all
                // following fixes for that string
          ...
          fixPositions.forEach((fixPosition) => {
            ...
            if (fixPosition.startIndex <
              ...
              node.params = replaceIndentation(
                  ...
                  fixPosition.currentIndentation,
                  fixPosition.expectedIndentation,
                  fixPosition.startIndex - atRuleName.length -
                      atRuleAfterName.length - 1,
              );
            }
          });
        }
...
function getRootBaseIndentLevel(root, baseIndentLevel, space) {
  ...
  if (!Number.isSafeInteger(indentLevel)) {
    indentLevel = inferRootIndentLevel(
        root,
        baseIndentLevel,
...
function inferDocIndentSize(document, space) {
  ...
  if (indents) {
    ...
    indents.forEach((leadingSpaces) => { vote(leadingSpaces.length); });
  ...
  indentSize = Number(indentSize) || (indents && indents[0].length) ||
               Number(space) || 2;
...
function inferRootIndentLevel(root, baseIndentLevel, indentSize) {
  ...
  // The indent level of the CSS code block in non-CSS-like files is determined
  // by the shortest indent of non-empty line.
  if (foundIndents) {
  ...
  if (after) {
    ...
    if (after.endsWith('\n')) {
      ...
      if (document) {
        ...
        afterEnd =
            nextRoot ? nextRoot.raws.beforeStart : document.raws.afterEnd;
      } else {
