  ...
  return (root, result) => {
    ...
    root.walkDecls((decl) => {
      ...
      parsedValue.walk((node) => {
        ...
        if (!LEGACY_NOTATION_FUNCS.has(value.toLowerCase()))
          return;

        if (primary === 'modern' && !hasCommas(node))
          return;

        if (primary === 'legacy' && hasCommas(node))
          return;
...
function isComma(node) { return node.type === 'div' && node.value === ','; }
