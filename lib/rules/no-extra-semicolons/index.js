  ...
  return (root, result) => {
    ...
    if (rawAfterRoot && rawAfterRoot.trim().length !== 0) {
      ...
      styleSearch({source : rawAfterRoot, target : ';'}, (match) => {
        ...
        complain(root.source.input.css.length - rawAfterRoot.length +
                 match.startIndex);
      });
    ...
    root.walk((node) => {
      ...
      if (node.type === 'rule' && !isCustomPropertySet(node) &&
      ...
      if (rawBeforeNode && rawBeforeNode.trim().length !== 0) {
        ...
        // Ignore semicolon before comment if next node is custom properties
        // sets or comment
        if (node.type === 'comment' && next && isCustomPropertySet(next) &&
        ...
        // Adding previous node string to custom properties set if previous node
        // is comment
        if (isCustomPropertySet(node) && node.parent.index(node) > 0 && prev &&
            prev.type === 'comment') {
        ...
        styleSearch({source : rawBeforeNode, target : ';'}, (match, count) => {
          ...
          complain(getOffsetByNode(node) - rawBeforeNode.length +
                   match.startIndex);
        });
      ...
      if (rawAfterNode && rawAfterNode.trim().length !== 0) {
        /**
         * If the last child is a Less mixin followed by more than one
         * semicolon, node.raws.after will be populated with that semicolon.
         * Since we ignore Less mixins, exit here
         */
        if (node.last && node.last.type === 'atrule' &&
        ...
        styleSearch({source : rawAfterNode, target : ';'}, (match) => {
          ...
          const index = getOffsetByNode(node) + node.toString().length - 1 -
                        rawAfterNode.length + match.startIndex;
      ...
      if (rawOwnSemicolon) {
        ...
        styleSearch(
            {source : rawOwnSemicolon, target : ';'}, (match, count) => {
              if (count === allowedSemi) {
                return;
              }
              ...
              const index = getOffsetByNode(node) + node.toString().length -
                            rawOwnSemicolon.length + match.startIndex;
        ...
        if (fixSemiIndices.length) {
          node.raws.ownSemicolon =
              removeIndices(rawOwnSemicolon, fixSemiIndices);
        }
      }
    });
    ...
    function removeIndices(str, indices) {
      indices.reverse().forEach(
          (index) => { str = str.slice(0, index) + str.slice(index + 1); });
