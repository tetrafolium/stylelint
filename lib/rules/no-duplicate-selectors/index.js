...
function rule(actual, options) {
  return (root, result) => {
    ...
    // Each source maps to another map, which maps rule parents to a set of
    // selectors. This ensures that selectors are only checked against selectors
    ...
    root.walkRules((rule) => {
      ...
      // doesn't matter
      const sortedSelectorList =
          normalizedSelectorList.slice().sort().join(',');
      ...
      if (previousDuplicatePosition) {
        ...
        // we have to approximate something for the message -- which is close
        // enough
        const isNestedSelector =
            resolvedSelectors.join(',') !== rule.selectors.join(',');
        const selectorForMessage =
            isNestedSelector ? resolvedSelectors.join(', ') : rule.selector;
        ...
        return report({
          ...
          node : rule,
          message :
              messages.rejected(selectorForMessage, previousDuplicatePosition),
        });
      }
      ...
      // Or complain if one selector list contains the same selector more than
      // once
