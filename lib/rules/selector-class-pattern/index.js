...
const messages = ruleMessages(ruleName, {
  expected : (selectorValue, pattern) => `Expected class selector ".${
      selectorValue}" to match pattern "${pattern}"`,
});
...
function rule(pattern, options) {
  return (root, result) => {
    ...
    const shouldResolveNestedSelectors =
        _.get(options, 'resolveNestedSelectors');
    const normalizedPattern =
        _.isString(pattern) ? new RegExp(pattern) : pattern;
...
function isCombinator(x) { return /[\s+>~]/.test(x); }
