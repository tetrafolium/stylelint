...
const isStandardSyntaxMathFunction =
    require('../../utils/isStandardSyntaxMathFunction');
...
const messages = ruleMessages(ruleName, {
  ...
  expectedValidResolvedType : (operator) =>
      `Expected to be compatible with the left and right argument types of "${
          operator}" operation.`,
});
      ...
      valueParser(decl.value).walk((node) => {
        ...
        try {
          ...
          if (e.hash && e.hash.loc) {
            complain(messages.expectedExpression(),
      ...
      function verifyMathExpressions(expression, node) {
        if (expression.type === 'MathExpression') {
          ...
          if (operator === '+' || operator === '-') {
            if (expression.source.operator.end.index ===
                right.source.start.index) {
              complain(
            ...
            if (expression.source.operator.start.index ===
                left.source.end.index) {
              complain(
...
function getNumber(mathExpression) {
  ...
  const leftValue =
      left.type === 'Value'
          ? left.value
          : left.type === 'MathExpression' ? getNumber(left) : null;
  const rightValue =
      right.type === 'Value'
          ? right.value
          : right.type === 'MathExpression' ? getNumber(right) : null;
...
function getResolvedType(mathExpression) {
  const {left : leftExpression, operator, right : rightExpression} =
      mathExpression;
  let left = leftExpression.type === 'MathExpression'
                 ? getResolvedType(leftExpression)
                 : leftExpression.type;
  let right = rightExpression.type === 'MathExpression'
