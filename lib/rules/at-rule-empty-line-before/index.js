...
const getPreviousNonSharedLineCommentNode =
    require('../../utils/getPreviousNonSharedLineCommentNode');
const hasEmptyLine = require('../../utils/hasEmptyLine');
const isAfterComment = require('../../utils/isAfterComment');
const isBlocklessAtRuleAfterBlocklessAtRule =
    require('../../utils/isBlocklessAtRuleAfterBlocklessAtRule');
const isBlocklessAtRuleAfterSameNameBlocklessAtRule =
    require('../../utils/isBlocklessAtRuleAfterSameNameBlocklessAtRule');
...
function rule(expectation, options, context) {
  return (root, result) => {
    ...
    root.walkAtRules((atRule) => {
      ...
      if (optionsMatches(options, 'ignore', 'blockless-after-blockless') &&
          isBlocklessAtRuleAfterBlocklessAtRule(atRule)) {
        return;
      }

      // Optionally ignore the node if it is the first nested
      if (optionsMatches(options, 'ignore', 'first-nested') &&
      ...
      if (optionsMatches(options, 'ignore',
                         'blockless-after-same-name-blockless') &&
      ...
      const message =
          expectEmptyLineBefore ? messages.expected : messages.rejected;
...
function isAtRuleAfterSameNameAtRule(atRule) {
  ...
  return previousNode && previousNode.type === 'atrule' &&
