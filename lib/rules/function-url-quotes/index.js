  ...
  return (root, result) => {
    ...
    function checkDeclParams(decl) {
      functionArgumentsSearch(
          decl.toString().toLowerCase(), 'url',
          (args, index) => { checkArgs(args, decl, index, 'url'); });
    }
    ...
    function checkAtRuleParams(atRule) {
      ...
      functionArgumentsSearch(
          atRuleParamsLowerCase, 'url-prefix', (args, index) => {
            checkArgs(args, atRule, index + atRuleParamIndex(atRule),
                      'url-prefix');
          });
      functionArgumentsSearch(
          atRuleParamsLowerCase, 'domain', (args, index) => {
    ...
    function checkArgs(args, node, index, functionName) {
      ...
      const hasQuotes =
          leftTrimmedArgs.startsWith("'") || leftTrimmedArgs.startsWith('"');
