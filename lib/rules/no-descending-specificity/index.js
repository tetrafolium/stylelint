...
const messages = ruleMessages(ruleName, {
  rejected : (b, a) =>
      `Expected selector "${b}" to come before selector "${a}"`,
});
...
function rule(on, options) {
  return (root, result) => {
    ...
    root.walkRules((rule) => {
      ...
      if (optionsMatches(options, 'ignore', 'selectors-within-list') &&
          rule.selectors.length > 1) {
        return;
      }

      const comparisonContext =
          selectorContextLookup.getContext(rule, findAtRuleContext(rule));
    ...
    function checkSelector(selectorNode, rule, sourceIndex, comparisonContext) {
      ...
      const referenceSelectorNode =
          lastCompoundSelectorWithoutPseudoClasses(selectorNode);
      const selectorSpecificity =
          specificity.calculate(selector)[0].specificityArray;
      ...
      const priorComparableSelectors =
          comparisonContext.get(referenceSelectorNode);

      priorComparableSelectors.forEach((priorEntry) => {
        if (specificity.compare(selectorSpecificity, priorEntry.specificity) ===
            -1) {
          report({
            ruleName,
            result,
...
function lastCompoundSelectorWithoutPseudoClasses(selectorNode) {
  const nodesAfterLastCombinator = _.last(
      selectorNode.nodes[0].split(
          (node) => { return node.type === 'combinator'; }),
  );

  const nodesWithoutPseudoClasses =
      nodesAfterLastCombinator
          .filter((node) => {
