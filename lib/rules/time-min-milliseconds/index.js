...
function rule(minimum, options) {
  return (root, result) => {
    ...
    root.walkDecls((decl) => {
      ...
      if (keywordSets.longhandTimeProperties.has(propertyName) &&
          !isIgnoredProperty(propertyName) && !isAcceptableTime(decl.value)) {
        complain(decl);
      }
    ...
    /**
     * Get the duration within an `animation` or `transition` shorthand property
     * value.
     *
    ...
    function isAcceptableTime(time) {
      ...
      if (parsedTime.unit.toLowerCase() === 'ms' &&
          parsedTime.number < minimum) {
        return false;
      }

      if (parsedTime.unit.toLowerCase() === 's' &&
