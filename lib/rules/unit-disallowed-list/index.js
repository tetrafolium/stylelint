...
const validateObjectWithArrayProps =
    require('../../utils/validateObjectWithArrayProps');
  ...
  return (root, result) => {
    const validOptions = validateOptions(
        ...
        },
        {
          ...
          possible : {
            ignoreProperties :
                validateObjectWithArrayProps([ _.isString, _.isRegExp ]),
    ...
    function checkDecl(node, value, getIndex) {
      ...
      valueParser(value).walk((valueNode) => {
        ...
        check(node, getIndex(node), valueNode, node.prop,
              options ? options.ignoreProperties : {});
      });
    }

    root.walkAtRules(/^media$/i, (atRule) => checkMedia(atRule, atRule.params,
                                                        atRuleParamIndex));
    root.walkDecls((decl) =>
                       checkDecl(decl, decl.value, declarationValueIndex));
  };
}
