  ...
  return (root, result) => {
    ...
    root.walkDecls((decl) => {
      ...
      parsedValue.walk((node, nodeIndex, nodes) => {
        if (decl.prop.toLowerCase() === 'font' && node.type === 'div' &&
            node.value === '/') {
    ...
    function check(value, node, ignorableIndexes = []) {
      ...
      styleSearch({source : value, target : '0'}, (match) => {
        ...
        // Given a 0 somewhere in the full property value (not in a string,
        // thanks to styleSearch) we need to isolate the value that contains the
        // zero. To do so, we'll find the last index before the 0 of a character
        // that would divide one value in a list from another, and the next
        // index of such a character; then we build a substring from those
        // indexes, which we can assess.

        // If a single value includes multiple 0's (e.g. 100.01px), we don't
        // want each 0 to be treated as a separate value, possibly resulting in
        // multiple warnings for the same value (e.g. 0.00px).
        //
        // This check prevents that from happening: we build and check against a
        // Set containing all the indexes that are part of a value already
        // validated.
        if (ignorableIndexes[index]) {
          return;
        }

        const prevValueBreakIndex =
            _.findLastIndex(value.substr(0, index), (char) => {
              return [ ' ', ',', ')', '(', '#', ':', '\n', '\t' ].includes(
                  char);
            });
        ...
        const nextValueBreakIndex =
            _.findIndex(value.substr(valueWithZeroStart), (char) => {
        ...
        _.range(valueWithZeroStart, valueWithZeroEnd)
            .forEach((i) => (ignorableIndexes[i] = true));

        // Only pay attention if the value parses to 0
        // and units with lengths
        if (Number.parseFloat(valueWithZero) !== 0 ||
      ...
      if (fixPositions.length) {
        fixPositions.forEach((fixPosition) => {
          if (node.type === 'atrule') {
            ...
            const realIndex = fixPosition.startIndex - node.name.length -
                              node.raws.afterName.length - 1;

            node.params =
                replaceZero(node.params, realIndex, fixPosition.length);
          } else {
            const realIndex = fixPosition.startIndex - node.prop.length -
