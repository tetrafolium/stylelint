...
function rule(expectation, options, context) {
  ...
  return (root, result) => {
    ...
    root.walkAtRules(/^media$/i, (atRule) => {
      ...
      const fix =
          context.fix ? (index) => fixOperatorIndices.push(index) : null;

      findMediaOperator(
          atRule, (match, params,
                   node) => { checkBeforeOperator(match, params, node, fix); });

      if (fixOperatorIndices.length) {
        let params =
            atRule.raws.params ? atRule.raws.params.raw : atRule.params;
