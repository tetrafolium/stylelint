  ...
  return (root, result) => {
    ...
    root.walkDecls((decl) => {
      valueParser(decl.value).walk((node) => {
        ...
        const expressionIndex = decl.source.start.column + decl.prop.length +
        ...
        function checkSymbol(symbol) {
          ...
          styleSearch(styleSearchOptions, (match) => {
            ...
            // Deal with signs.
            // (@ and $ are considered "digits" here to allow for variable
            // syntaxes that permit signs in front of variables, e.g.
            // `-$number`) As is "." to deal with fractional numbers without a
            // leading zero
            if ((symbol === '+' || symbol === '-') &&
              ...
              // And if not, complain
              complain(messages.expectedOperatorBeforeSign(symbol), decl,
                       expressionIndex + index);

              return;
            }

            const beforeOk = (expression[index - 1] === ' ' &&
                              !isWhitespace(expression[index - 2])) ||
                             newlineBefore(expression, index - 1);

            if (!beforeOk) {
              complain(messages.expectedBefore(symbol), decl,
                       expressionIndex + index);
            }

            const afterOk = (expression[index + 1] === ' ' &&
                             !isWhitespace(expression[index + 2])) ||
                            expression[index + 1] === '\n' ||
                            expression.substr(index + 1, 2) === '\r\n';

            if (!afterOk) {
              complain(messages.expectedAfter(symbol), decl,
                       expressionIndex + index);
            }
          });
        }
