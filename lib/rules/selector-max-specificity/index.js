...
const isStandardSyntaxSelector =
    require('../../utils/isStandardSyntaxSelector');
...
const messages = ruleMessages(ruleName, {
  ...
  expected : (selector, specificity) => `Expected "${
      selector}" to have a specificity no more than "${specificity}"`,
});

// Return an array representation of zero specificity. We need a new array each
// time so that it can mutated
...
const specificitySum = (specificities) => {
  ...
  specificities.forEach((specificityArray) => {
    specificityArray.forEach((value, i) => { sum[i] += value; });
  });
...
function rule(max, options) {
  return (root, result) => {
    ...
    // Calculate the specificity of a simple selector (type, attribute, class,
    // ID, or pseudos's own value)
    ...
    const maxChildSpecificity = (node) =>
        ...
        node.reduce((max, child) => {
          const childSpecificity = nodeSpecificity(
              child); // eslint-disable-line no-use-before-define
    ...
    // Calculate the specificity of a pseudo selector including own value and
    // children
    const pseudoSpecificity = (node) => {
      // `node.toString()` includes children which should be processed
      // separately, so use `node.value` instead
      const ownValue = node.value;
      const ownSpecificity = ownValue === ':not' || ownValue === ':matches'
                                 ? // :not and :matches don't add specificity
                                   // themselves, but their children do
                                 zeroSpecificity()
    ...
    const shouldSkipPseudoClassArgument = (node) => {
      ...
      if (parentNode && parentNode.value) {
        ...
        const normalisedParentNode =
            parentNodeValue.toLowerCase().replace(/:+/, '');

        return (
            parentNode.type === 'pseudo' &&
            (keywordSets.aNPlusBNotationPseudoClasses.has(
                 normalisedParentNode) ||
    ...
    root.walkRules((rule) => {
      ...
      // Using rule.selectors gets us each selector in the eventuality we have a
      // comma separated set
        ...
        resolvedNestedSelector(selector, rule).forEach((resolvedSelector) => {
          try {
            ...
            parseSelector(resolvedSelector, result, rule, (selectorTree) => {
              ...
              if (specificity.compare(maxChildSpecificity(selectorTree),
                                      maxSpecificityArray) === 1) {
                report({
                  ruleName,
                  result,
                  node : rule,
