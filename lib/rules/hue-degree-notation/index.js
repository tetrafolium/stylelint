  ...
  return (root, result) => {
    ...
    root.walkDecls((decl) => {
      ...
      parsedValue.walk((node) => {
        ...
        if (!HUE_FUNCS.has(node.value.toLowerCase()))
          return;
        ...
        if (!isDegree(value) && !isNumber(value))
          return;

        if (primary === 'angle' && isDegree(value))
          return;

        if (primary === 'number' && isNumber(value))
          return;
...
function asDegree(value) { return `${value}deg`; }
