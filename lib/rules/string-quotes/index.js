  ...
  return (root, result) => {
          ...
          if (attributeNode.quoteMark === correctQuote) {
            if (avoidEscape) {
              const needsCorrectEscape =
                  attributeNode.value.includes(correctQuote);
              const needsOtherEscape =
                  attributeNode.value.includes(erroneousQuote);
              ...
              if (needsCorrectEscape) {
                if (context.fix) {
                  ...
                  report({
                    ...
                    node : rule,
                    index : attributeNode.sourceIndex +
    ...
    function checkDeclOrAtRule(node, value, getIndex) {
      ...
      if (node.type === 'atrule' && node.name === 'charset') {
        // allow @charset rules to have double quotes, in spite of the
        // configuration
        // TODO: @charset should always use double-quotes, see
        // https://github.com/stylelint/stylelint/issues/2788
        return;
      }
