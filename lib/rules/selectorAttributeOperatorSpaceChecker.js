...
module.exports = function(options) {
  options.root.walkRules((rule) => {
    ...
    const fixedSelector =
        parseSelector(selector, options.result, rule, (selectorTree) => {
          selectorTree.walkAttributes((attributeNode) => {
            ...
            styleSearch({source : attributeNodeString, target : operator},
                        (match) => {
                          const index = options.checkBeforeOperator
                                            ? match.startIndex
                                            : match.endIndex - 1;

                          checkOperator(attributeNodeString, index, rule,
    ...
    function checkOperator(source, index, node, attributeNode, operator) {
      options.locationChecker({
        ...
        err : (m) => {
          ...
          report({
            message : m.replace(
                options.checkBeforeOperator ? operator[0]
