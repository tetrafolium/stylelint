...
const ALPHA_FUNCS =
    new Set([ 'hsl', 'hsla', 'hwb', 'lab', 'lch', 'rgb', 'rgba' ]);
    ...
    root.walkDecls((decl) => {
      ...
      parsedValue.walk((node) => {
        ...
        if (ALPHA_PROPS.has(decl.prop.toLowerCase())) {
          ...
          if (!ALPHA_FUNCS.has(node.value.toLowerCase()))
            return;
        ...
        if (!isNumber(value) && !isPercentage(value))
          return;
        ...
        if (optionsMatches(options, 'exceptProperties', decl.prop)) {
          expectation =
              Object.keys(optionFuncs).filter((key) => key !== expectation);
        }

        if (optionFuncs[expectation].expFunc(value))
          return;
...
function findAlphaInFunction(node) {
  const args =
      node.nodes.filter(({type}) => type === 'word' || type === 'function');
