...
function rule(expectation, options, context) {
      ...
      const fixedSelector =
          parseSelector(selector, result, rule, (selectorTree) => {
            selectorTree.walkAttributes((attributeNode) => {
              ...
              styleSearch(
                  {source : attributeSelectorString, target : '['}, (match) => {
                    const nextCharIsSpace =
                        attributeSelectorString[match.startIndex + 1] === ' ';
                    const index =
                        attributeNode.sourceIndex + match.startIndex + 1;
  ...
  function fixBefore(attributeNode) {
    ...
    const {attrBefore, setAttrBefore} =
        rawAttrBefore ? {
          attrBefore : rawAttrBefore,
          setAttrBefore(
              fixed) { attributeNode.raws.spaces.attribute.before = fixed; },
        }
                      : {
                          attrBefore : _.get(attributeNode,
                                             'spaces.attribute.before', ''),
                          setAttrBefore(fixed) {
                            _.set(attributeNode, 'spaces.attribute.before',
                                  fixed);
                          },
                        };
  ...
  function fixAfter(attributeNode) {
    ...
    const {after, setAfter} =
        rawAfter ? {
          after : rawAfter,
          setAfter(fixed) { attributeNode.raws.spaces[key].after = fixed; },
        }
                 : {
