    ...
    root.walkRules((rule) => {
      ...
      parseSelector(selector, result, rule, (selectorAST) => {
        selectorAST.walkTags((tag) => {
          ...
          const expectedValue = expectation === 'lower' ? value.toLowerCase()
                                                        : value.toUpperCase();
          ...
          if (context.fix) {
            if (hasComments) {
              hasComments = hasComments.slice(0, sourceIndex) + expectedValue +
                            hasComments.slice(sourceIndex + value.length);
              _.set(rule, 'raws.selector.raw', hasComments);
            } else {
              rule.selector = rule.selector.slice(0, sourceIndex) +
                              expectedValue +
