...
const messages = ruleMessages(ruleName, {
  expected : (max) => `Expected line length to be no more than ${max} ${
...
function rule(maxLength, options, context) {
  return (root, result) => {
    ...
    EXCLUDED_PATTERNS.forEach(
        (pattern) => execall(pattern, rootString).forEach((match) => {
          const startOfSubString =
              match.index +
              match.match.indexOf(_.get(match, 'subMatches[0]', ''));
    ...
    function tryToPopSubString(start, end) {
      const [startSubString, endSubString] =
          skippedSubStrings[skippedSubStringsIndex];
    ...
    function checkNewline(rootString, match, root) {
      ...
      if (ignoreComments) {
        ...
        //       /* something here */
        const nextTwoChars =
            rootString.slice(match.endIndex).trim().slice(0, 2);
