...
const messages = ruleMessages(ruleName, {
  rejected : (selector) =>
      `Unexpected unknown pseudo-class selector "${selector}"`,
});
...
function rule(actual, options) {
  return (root, result) => {
    ...
    function check(selector, result, node) {
      parseSelector(selector, result, node, (selectorTree) => {
        selectorTree.walkPseudos((pseudoNode) => {
          ...
          if (optionsMatches(options, 'ignorePseudoClasses',
          ...
          if (node.type === 'atrule' && node.name === 'page') {
            ...
            if (vendor.prefix(name) || keywordSets.pseudoClasses.has(name) ||
            ...
            if (prevPseudoElement) {
              ...
              if (keywordSets.webkitProprietaryPseudoElements.has(
    ...
    root.walk((node) => {
      ...
      // Return if selector empty, it is meaning node type is not a rule or a
      // at-rule
