...
const isStandardSyntaxFunction =
    require('../../utils/isStandardSyntaxFunction');
  ...
  return (root, result) => {
    ...
    root.walkDecls((decl) => {
      ...
      valueParser(decl.value).walk((node) => {
        ...
        if (expectation === 'never' && type === 'word' &&
            namedColors.includes(value.toLowerCase())) {
          complain(messages.rejected(value), decl,
        ...
        if (type === 'function' &&
          ...
          const normalizedFunctionString =
              valueParser.stringify(node).replace(/\s+/g, '');
          let namedColor;

          for (let i = 0, l = namedColors.length; i < l; i++) {
            namedColor = namedColors[i];

            if (namedColorData[namedColor].func.includes(
