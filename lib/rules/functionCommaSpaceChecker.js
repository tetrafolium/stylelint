...
module.exports = function(opts) {
  opts.root.walkDecls((decl) => {
    ...
    parsedValue.walk((valueNode) => {
      ...
      // Ignore `url()` arguments, which may contain data URIs or other funky
      // stuff
      ...
      const argumentStrings =
          valueNode.nodes.map((node) => valueParser.stringify(node));
      ...
      const functionArguments = (() => {
        ...
        // 1. Remove comments including preceding whitespace (when only
        // succeeded by whitespace)
      ...
      function getCommaCheckIndex(commaNode, nodeIndex) {
        let commaBefore = valueNode.before +
      ...
      for (const {commaNode, checkIndex, nodeIndex} of commaDataList) {
        opts.locationChecker({
          ...
          index : checkIndex,
          err : (message) => {
            const index = declarationValueIndex(decl) + commaNode.sourceIndex +
