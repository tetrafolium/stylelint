...
const isStandardSyntaxProperty =
    require('../../utils/isStandardSyntaxProperty');
  ...
  return (root, result) => {
    ...
    root.walkDecls((decl) => {
      if (!isStandardSyntaxDeclaration(decl) ||
          !isStandardSyntaxProperty(decl.prop) || !decl.value.startsWith('-')) {
        return;
      }
      ...
      styleSearch({source : value.toLowerCase(), target : valuePrefixes},
                  (match) => {
                    const fullIdentifier =
                        /^(-[a-z-]+)\b/i.exec(value.slice(match.startIndex))[1];
                    ...
                    report({
                      ...
                      node : decl,
                      index : prop.length + (decl.raws.between || '').length +
                                  match.startIndex,
                      result,
                      ruleName,
                    });
