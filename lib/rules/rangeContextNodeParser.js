...
function getRangeContextName(parsedNode) {
  ...
  // The parsedNode is ["10em", "width", "50em"] - the name is always in the
  // second position
  ...
  // Regex is needed because the name can either be in the first or second
  // position
  return parsedNode.find((value) => value.match(/^(?!--)\D+/) ||
                                    value.match(/^(--).+/));
}
...
module.exports = function(node) {
  ...
  const parsedMedia =
      nodeValue.replace(/[()\s]/g, '').split(new RegExp(operators.join('|')));
  ...
  const values =
      parsedMedia.filter((parsedValue) => parsedValue !== name).map((value) => {
        return {
          value,
