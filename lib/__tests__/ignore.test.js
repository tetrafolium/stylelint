...
test(
    'same as above with no configBasedir, ignore-files path relative to process.cwd',
    () => {
...
test('specified `ignorePattern` file ignoring two files', () => {
  const files = [
    `${fixturesPath}/empty-block.css`, `${fixturesPath}/no-syntax-error.css`
  ];
  const noFilesErrorMessage = new NoFilesFoundError(files);
  const actualCwd = process.cwd();
  ...
  return standalone({
           ...
           ignorePattern :
               [ 'fixtures/empty-block.css', 'fixtures/no-syntax-error.css' ],
         })
      .catch((actualError) => {
        // no files read
        expect(actualError).toEqual(noFilesErrorMessage);
...
test(
    'using ignoreFiles with input files that would cause a postcss syntax error',
    () => {
      return standalone({
          ...
          .then(({results}) => {
            ...
            const one = results.find(
                (result) => result.source.includes('no-syntax-error.css'));
            const two = results.find((result) => result.source.includes(
                                         'syntax-error-ignored.scss'));
