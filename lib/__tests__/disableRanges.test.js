...
it('enable all without disabling any', async () => {
  return expect(testDisableRanges('/* stylelint-enable */'))
      .rejects.toThrow(
...
it('enable rule without disabling any', async () => {
  return expect(testDisableRanges('/* stylelint-enable foo */'))
      .rejects.toThrow(
...
function testDisableRanges(source, options) {
  return postcss().use(assignDisabledRanges).process(source, {
    from : undefined,
    ...options
  });
}
