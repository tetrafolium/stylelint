...
it('one plugin runs', () => {
  return processorRelative.process(cssWithFoo, {from : undefined})
      .then((result) => {
        expect(result.warnings()).toHaveLength(2);
        expect(result.warnings()[0].text)
            .toBe('found .foo (plugin/warn-about-foo)');
        expect(result.warnings()[0].node).toBeTruthy();
      });
});

it('another plugin runs', () => {
  return processorRelative.process(cssWithoutFoo, {from : undefined})
      .then((result) => {
        expect(result.warnings()).toHaveLength(1);
        expect(result.warnings()[0].text)
            .toBe('Unexpected empty block (block-no-empty)');
      });
});

it('plugin with absolute path and no configBasedir', () => {
  return processorAbsolute.process(cssWithFoo, {from : undefined})
      .then((result) => {
        expect(result.warnings()).toHaveLength(2);
        expect(result.warnings()[0].text)
            .toBe('found .foo (plugin/warn-about-foo)');
...
it('config extending another config that invokes a plugin with a relative path',
   () => {
     return processorExtendRelative.process(cssWithFoo, {from : undefined})
         .then((result) => {
           expect(result.warnings()).toHaveLength(1);
           expect(result.warnings()[0].text)
               .toBe('found .foo (plugin/warn-about-foo)');
...
describe('plugin using exposed rules via stylelint.rules', () => {
  ...
  const config = (expectation) => ({
    config : {
      plugins : [ path.join(
          __dirname, 'fixtures/plugin-conditionally-check-color-hex-case') ],
      rules : {
  ...
  it('with uppercase expectation and lowercase color', () => {
    return postcss()
        ...
        .then((result) => {
          expect(result.warnings()).toHaveLength(1);
          expect(result.warnings()[0].text)
              .toBe('Expected "#eee" to be "#EEE" (color-hex-case)');
        });
  });
  ...
  it('with uppercase expectation and uppercase color', () => {
    return postcss()
        ...
        .then((result) => { expect(result.warnings()).toHaveLength(0); });
  });
...
it('slashless plugin causes configuration error', () => {
  const config = {
    plugins :
        [ path.join(__dirname, 'fixtures/plugin-slashless-warn-about-foo') ],
    rules : {
...
it('plugin with async rule', () => {
  ...
  return postcss()
      .use(stylelint(config))
      .process('a {}', {from : undefined})
      .then((result) => { expect(result.warnings()).toHaveLength(1); });
});
...
describe('loading a plugin from process.cwd', () => {
  ...
  it('error is caught', () => { expect(result.warnings()).toHaveLength(1); });
