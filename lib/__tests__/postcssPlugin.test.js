...
it('`config` option is `null`', () => {
  return postcssPlugin.process('a {}', {from : undefined})
...
it('`configFile` option with absolute path', () => {
  ...
  return postcssPlugin.process('a {}', {from : undefined}, config)
      .then((postcssResult) => {
...
it('`configFile` option with undefined rule', () => {
  const config = {
    configFile :
        path.join(__dirname, 'fixtures/config-with-undefined-rule.json'),
  };
...
describe('stylelintignore', () => {
  ...
  afterEach(() => { process.chdir(actualCwd); });
  ...
  it('postcssPlugin with .stylelintignore and file is ignored', () => {
    ...
    return postcssPlugin.process('a {}', {from : undefined}, options)
        .then((postcssResult) => {
          expect(postcssResult.stylelint.ignored).toBeTruthy();
        });
  });
  ...
  it('postcssPlugin with ignorePath and file is ignored', () => {
    const options = {
      ...
      from : 'foo.css',
      ignorePath :
          path.join(__dirname, './stylelintignore-test/.postcssPluginignore'),
    };
