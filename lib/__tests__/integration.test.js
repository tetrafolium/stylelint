...
describe('integration test expecting warnings', () => {
  ...
  it('block-opening-brace-newline-after - string primary option', () => {
    ...
    expect(error.text)
        .toBe('Expected newline after "{" (block-opening-brace-newline-after)');
  ...
  it('color-no-invalid-hex - true primary option', () => {
    const errors =
        result.messages.filter((msg) => msg.rule === 'color-no-invalid-hex');
  ...
  it('function-disallowed-list - array primary option', () => {
    const error =
        result.messages.find((msg) => msg.rule === 'function-disallowed-list');

    expect(error).toBeTruthy();
    expect(error.text)
        .toBe('Unexpected function "calc" (function-disallowed-list)');
    expect(error.severity).toBe('error');
  });
});
...
it('Less integration test', () => {
  ...
  return postcss()
      ...
      .then((result) => { expect(result.messages).toHaveLength(0); });
});
...
describe('integration test null option', () => {
  ...
  it('no warnings', () => { expect(results[0].warnings).toHaveLength(0); });
});
