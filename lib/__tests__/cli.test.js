...
describe('buildCLI', () => {
  ...
  it('flags.allowEmptyInput', () => {
    expect(buildCLI([ '--allow-empty-input' ]).flags.allowEmptyInput)
        .toBe(true);
  ...
  it('flags.cacheLocation', () => {
    expect(buildCLI([ '--cache-location=foo' ]).flags.cacheLocation)
        .toBe('foo');
  });
  ...
  it('flags.config', () => {
    expect(buildCLI([ '--config=/path/to/file' ]).flags.config)
        .toBe('/path/to/file');
  });
  ...
  it('flags.customFormatter', () => {
    expect(buildCLI([ '--custom-formatter=foo' ]).flags.customFormatter)
        .toBe('foo');
  });
  ...
  it('flags.disableDefaultIgnores', () => {
    expect(
        buildCLI([ '--disable-default-ignores' ]).flags.disableDefaultIgnores)
        .toBe(true);
  ...
  it('flags.ignorePath', () => {
    expect(buildCLI([ '--ignore-path=/path/to/file' ]).flags.ignorePath)
        .toBe('/path/to/file');
    expect(buildCLI([ '-i', '/path/to/file' ]).flags.ignorePath)
        .toBe('/path/to/file');
  });

  it('flags.ignorePattern', () => {
    expect(buildCLI([ '--ignore-pattern=vendor/**' ]).flags.ignorePattern)
        .toEqual([ 'vendor/**' ]);
    expect(buildCLI([ '--ip', 'vendor/**' ]).flags.ignorePattern).toEqual([
      'vendor/**'
    ]);
    expect(buildCLI([
             '--ip', 'vendor/**', '--ip', 'test/*.css'
           ]).flags.ignorePattern)
        .toEqual([
          'vendor/**',
          'test/*.css',
  ...
  it('flags.outputFile', () => {
    expect(buildCLI([ '--output-file=/path/to/file' ]).flags.outputFile)
        .toBe('/path/to/file');
    expect(buildCLI([ '-o', '/path/to/file' ]).flags.outputFile)
        .toBe('/path/to/file');
  });
  ...
  it('flags.reportInvalidScopeDisables', () => {
    expect(buildCLI([
           ...
           ]).flags.reportInvalidScopeDisables)
        .toBe(
            true,
        );
  ...
  it('flags.reportNeedlessDisables', () => {
    expect(
        buildCLI([ '--report-needless-disables' ]).flags.reportNeedlessDisables)
        .toBe(true);
  ...
  it('flags.stdinFilename', () => {
    expect(buildCLI([ '--stdin-filename=foo.css' ]).flags.stdinFilename)
        .toBe('foo.css');
  });
...
describe('CLI', () => {
  ...
  it('--version', async () => {
    ...
    expect(console.log).toHaveBeenCalledTimes(1);
    expect(console.log)
        .toHaveBeenCalledWith(expect.stringContaining(pkg.version));
  });
    ...
    expect(process.stdout.write)
        .toHaveBeenCalledWith(
            expect.stringMatching(
                /Needless disable for "color-named".*Unexpected empty block/s),
        );
  });
  ...
  it('reports disallowed disables', async () => {
    await cli([
      '--config',
      replaceBackslashes(path.join(
          fixturesPath, 'config-block-no-empty-report-disables.json')),
      replaceBackslashes(
          path.join(fixturesPath, 'empty-block-with-relevant-disable.css')),
    ]);
    ...
    expect(process.stdout.write)
        .toHaveBeenCalledWith(
            expect.stringContaining(
                'Rule "block-no-empty" may not be disabled'),
        );
  });
  ...
  it('--stdin', async () => {
    await cli([
      '--stdin', '--config', `${fixturesPath}/config-no-empty-source.json`
    ]);
