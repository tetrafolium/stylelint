...
it('[normalized rule settings] primary option array', () => {
  return standalone({
      ...
      .then((linted) => {
        expect(linted.results[0].warnings[0].rule)
            .toBe('selector-pseudo-class-disallowed-list');
      });
});
...
it('[normalized rule settings] array primary, primary option null', () => {
  return standalone({
           ...
           config : {
             extends :
                 [ path.join(__dirname, 'fixtures/config-no-pixels.json') ],
             rules : {
