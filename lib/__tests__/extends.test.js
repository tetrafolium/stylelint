...
const configExtendingAnotherExtend =
    require('./fixtures/config-extending-another-extend.json');
const configExtendingOne = require('./fixtures/config-extending-one');
const configExtendingThreeWithOverride =
    require('./fixtures/config-extending-three-with-override');
...
it('extending with overrides', () => {
  return standalone({
      ...
      .then(
          (linted) => { expect(linted.results[0].warnings).toHaveLength(0); });
});
...
it('extending configuration and no configBasedir', () => {
  return standalone({
      ...
      .catch((err) => { expect(err.code).toBe(78); });
});
