...
describe('standalone with one input file', () => {
  ...
  beforeEach(() => {
    return standalone({
             ...
             // Path to config file
             configFile :
                 path.join(__dirname, 'fixtures/config-block-no-empty.json'),
           })
        .then((data) => {
          output = data.output;
...
it('standalone with two file-specific globs', async () => {
  ...
  /**
   * Expecting warnings for these two rules, and order of warnings is not
   * important, because files could be linted in any order
   * [
...
it('standalone without input css and file(s) should throw error', () => {
  ...
  expect(() => standalone({config : configBlockNoEmpty}))
...
it('standalone with non-existent-file throws an error', () => {
  ...
  return standalone({
      ...
      .catch((actualError) => { expect(actualError).toEqual(expectedError); });
});
...
describe('standalone passing code with syntax error', () => {
  ...
  it('error registered', () => { expect(results[0].errored).toBeTruthy(); });
...
it('syntax error sets errored to true', () => {
  return standalone({
      ...
      .then((linted) => { expect(linted.errored).toBe(true); });
});
...
it('unknown formatter option', () => {
  return standalone({
      ...
      .then(() => { throw new Error('should not have succeeded'); })
      .catch((err) => {
        expect(err.message.startsWith('You must use a valid formatter option'))
            .toBe(true);
      });
});
...
describe('standalone with config locatable from process.cwd not file', () => {
  ...
  afterAll(() => { process.chdir(actualCwd); });

  beforeEach(() => {
    return standalone({
             files : [ replaceBackslashes(
                 path.join(__dirname, './fixtures/empty-block.css')) ],
           })
        .then((data) => (results = data.results));
  });

  it('two warning', () => { expect(results[0].warnings).toHaveLength(2); });

  it("'block-no-empty' correct warning", () => {
    expect(results[0].warnings.find((warn) => warn.rule === 'block-no-empty'))
        .toBeTruthy();
  });

  it("'plugin/warn-about-foo' correct warning", () => {
    expect(results[0].warnings.find((warn) =>
                                        warn.rule === 'plugin/warn-about-foo'))
        .toBeTruthy();
  });
});
  ...
  it('Setting `plugins` inside `**/__tests__/**` object should override the ones set in `config` object',
     () => {
       return standalone({
           ...
           .then((linted) => {
             expect(linted.results[0].warnings).toHaveLength(1);
             expect(linted.results[0].warnings[0].text)
                 .toBe('found .bar (plugin/warn-about-bar)');
           });
     });
