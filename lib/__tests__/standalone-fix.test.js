...
const {promises : fs} = require(
    'fs'); // eslint-disable-line node/no-unsupported-features/node-builtins
...
it('outputs fixed code when input is code string', () => {
  return standalone({
      ...
      .then((result) => { expect(result.output).toBe('a { color: red; }'); });
});

it('does not modify shorthand object syntax when autofixing', () => {
  const codeString =
      `const width = '100px'; const x = <div style={{width}}>Hi</div>`;
  ...
  return standalone({
      ...
      .then((result) => { expect(result.output).toBe(codeString); });
});
...
it("doesn't fix with stylelint-disable commands", () => {
  ...
  return standalone({
      ...
      .then((result) => { expect(result.output).toBe(code); });
});
...
describe('writing fixes to files', () => {
  ...
  beforeEach(() => {
    tmpDir = os.tmpdir();
    tempFile =
        replaceBackslashes(path.join(tmpDir, `stylesheet-${_.uniqueId()}.css`));

    return fs.copyFile(`${fixturesPath}/fix.css`, tempFile);
  });

  afterEach(() => { return del(tempFile, {force : true}); });
    ...
    return standalone({
        ...
        .then(() => {
          return fs.readFile(tempFile, 'utf8').then((newFile) => {
            return fs.readFile(`${fixturesPath}/fix.css`, 'utf8')
                .then((oldFile) => { expect(newFile).toBe(oldFile); });
          });
        });
  });
