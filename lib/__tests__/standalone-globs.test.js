...
const fixturesPath =
    replaceBackslashes(path.join(__dirname, 'fixtures', 'globs'));
...
describe('standalone globbing', () => {
  describe('paths with special characters', () => {
    ...
    const fixtureDirs = [
      `[digit]/not-digits`, `a(b)`, `with spaces`, `extglob!(s)`,
      `got!negate/negate`,
      // `extglob+(s)`, // Note: +'s cause errors. Ignoring until it becomes a
      // problem
    ];
  ...
  it('glob has no + character, matched path does', async () => {
    const files =
        `${fixturesPath}/**/glob-plus.css`; // file is in dir 'glob+chars'
  ...
  it('negated glob patterns', async () => {
    ...
    expect(results[0].source)
        .toEqual(expect.stringContaining('lint-this-file.css'));
  ...
  describe('mixed globs and paths with special chars', () => {
    it('manual escaping', async () => {
      const cssGlob =
          `${fixturesPath}/got\\[braces\\] and \\(spaces\\)/*.+(s|c)ss`;
    ...
    /* eslint-disable jest/no-commented-out-tests -- Failing case for reference.
     * Documents behaviour that doesn't work. */

    // Note: This fails because there's no way to tell which parts of the glob
    // are literal characters, and which are special globbing characters.
    //
    // 'got[braces] and (spaces)' is a literal directory path. `*.+(s|c)ss` is a
    // glob.

    // https://github.com/stylelint/stylelint/issues/4855
    // it('glob and matched path contain different special chars, complex
    // example', async () => { 	const cssGlob = `${fixturesPath}/got[braces] and
    // (spaces)/*.+(s|c)ss`;
