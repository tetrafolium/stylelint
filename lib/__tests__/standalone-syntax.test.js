...
it('standalone with postcss-html syntax', () => {
  ...
  return standalone({
      ...
      .then((linted) => {
        ...
        const atRuleEmptyLineBeforeResult = results.find(
            (r) => /[/\\]at-rule-empty-line-before\.html$/.test(r.source),
        );

        expect(atRuleEmptyLineBeforeResult.errored).toBeFalsy();
        expect(atRuleEmptyLineBeforeResult.warnings).toHaveLength(0);

        const commentEmptyLineBeforeResult = results.find(
            (r) => /[/\\]comment-empty-line-before\.html$/.test(r.source),
        );

        expect(commentEmptyLineBeforeResult.errored).toBeFalsy();
        expect(commentEmptyLineBeforeResult.warnings).toHaveLength(0);

        const noEmptySourceResult =
            results.find((r) => /[/\\]no-empty-source\.html$/.test(r.source));

        expect(noEmptySourceResult.errored).toBeFalsy();
        expect(noEmptySourceResult.warnings).toHaveLength(0);

        const ruleEmptyLineBeforeResult = results.find(
            (r) => /[/\\]rule-empty-line-before\.html$/.test(r.source),
        );
        ...
        expect(ruleEmptyLineBeforeResult.warnings[0].rule)
            .toBe('rule-empty-line-before');
      });
});
  ...
  it('By extension', () => {
    return standalone({
        ...
        .then(({results}) => {
          ...
          const sssResult =
              results.find((r) => path.extname(r.source) === '.sss');
          const lessResult =
              results.find((r) => path.extname(r.source) === '.less');
          const sassResult =
              results.find((r) => path.extname(r.source) === '.sass');
          const scssResult =
              results.find((r) => path.extname(r.source) === '.scss');
          const htmlResult =
              results.find((r) => path.extname(r.source) === '.html');
          const jsResult =
              results.find((r) => path.extname(r.source) === '.ts');
  ...
  return standalone({
      ...
      .then((data) => {
        ...
        const htmlResult =
            results.find((r) => path.extname(r.source) === '.html');
        const vueResult =
            results.find((r) => path.extname(r.source) === '.vue');
        const mdResult =
            results.find((r) => path.extname(r.source) === '.markdown');

        expect(htmlResult.warnings[0].line).toBe(8);
        expect(vueResult.warnings[0].line).toBe(3);
        expect(mdResult.warnings[0].line).toBe(6);
        results.forEach((result) => {
          expect(result._postcssResult.css)
              .toEqual(result._postcssResult.root.source.input.css);
          expect(result.warnings).toHaveLength(1);
...
it('standalone with js file contains html', () => {
  return standalone({
      ...
      .then((data) => { expect(data.results[0].errored).toBe(false); });
});
  ...
  return standalone({
      ...
      .then((data) => {
        ...
        return Promise.all(
            results
                .filter((result) =>
                            safeParserExtensionsTest.test(result.source))
                .map((result) => {
                  ...
                  return promisify(fs.writeFile)(root.source.input.file,
