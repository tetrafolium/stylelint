...
const fileExists = (filePath) => new Promise(
    (resolve) => fs.access(filePath, fs.F_OK, (error) => resolve(!error)));
...
describe('standalone cache', () => {
  ...
  afterEach(() => {
    ...
    return Promise.all([
      unlink(expectedCacheFilePath)
          .catch(() => {
                     // fs.unlink() throws an error if file doesn't exist and
                     // it's ok. We just
  ...
  it('only changed files are linted', () => {
    ...
    return copyFile(validFile, newFileDest)
        ...
        .then((output) => {
          ...
          const isValidFileLinted =
              Boolean(output.results.find((file) => file.source === validFile));
          const isNewFileLinted = Boolean(
          ...
          .then(() => {
            throw new Error(
                `Cache file is supposed to be removed, ${
                    expectedCacheFilePath} is found instead`,
            );
          })
          .catch(() => {
...
describe('standalone cache uses cacheLocation', () => {
  ...
  const expectedCacheFilePath =
      path.join(cacheLocationDir, `.stylelintcache_${hash(cwd)}`);
  ...
  it('cacheLocation is a directory', () => {
    ...
    return standalone(config)
        .then(() => { return fileExists(expectedCacheFilePath); })
        .then((cacheFileStats) => {
          // Ensure cache file is created
