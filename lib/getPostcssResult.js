...
module.exports = function(stylelint, options = {}) {
  ...
  return getCode
      .then((code) => {
        ...
        if (stylelint._options.customSyntax) {
          ...
          if (stylelint._options.syntax === 'css') {
            ...
            if (!keys.includes(stylelint._options.syntax)) {
              throw new Error(
                  `You must use a valid syntax option, either: css, ${
                      keys.slice(0, -1).join(', ')} or ${keys.slice(-1)}`,
              );
            }
        ...
        const result =
            new LazyResult(postcssProcessor, preProcessedCode, postcssOptions);
...
function getCustomSyntax(customSyntax) {
  ...
  if (typeof customSyntax === 'string') {
    try {
      ...
      throw new Error(
          `Cannot resolve custom syntax module ${
              customSyntax}. Check that module ${
              customSyntax} is available and spelled correctly.`,
      );
    }
...
function cssSyntax(stylelint) {
  return {
    parse : stylelint._options.fix ? require('postcss-safe-parser')
                                   : postcss.parse,
    stringify : postcss.stringify,
  };
}
