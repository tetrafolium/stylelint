...
module.exports = function(valueNode) {
  ...
  // Map without comments and spaces will have the structure like $map (prop:
  // value, prop2: value)
  //                                                                     ↑  ↑ ↑
  //                                                                     ↑ 0  1
  //                                                                     2  3
  if (cleanNodes[0] && cleanNodes[0].type !== 'word' &&
      cleanNodes[0].type !== 'string') {
    return false;
  }
  ...
  // There is no need to check type or value of this node since it could be
  // anything
  if (!cleanNodes[2]) {
    return false;
  }
