...
module.exports = {
  ...
  selector(identifier) {
    return prefixes.remove.selectors.some(
        (/** @type {{ prefixed: string}} */ selectorObj) => {
  ...
  property(identifier) {
    return Boolean(
        autoprefixer.data
            .prefixes[prefixes.unprefixed(identifier.toLowerCase())]);
  },
  ...
  propertyValue(prop, value) {
    ...
    return (possiblePrefixableValues &&
            possiblePrefixableValues.some(
                (/** @type {{ prefixed: string}} */ valueObj) => {
   ...
   * @returns {string}
   */
  unprefix(value) { return value.replace(/-\w+-/, ''); },
};
