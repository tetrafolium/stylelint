...
module.exports = function(source, functionName, blurChar = '`') {
  ...
  while (lowerCaseSource.includes(nameWithParen, searchStartIndex)) {
    const openingParenIndex =
        ...
        functionNameLength;
    const parensMatch =
        balancedMatch('(', ')', lowerCaseSource.slice(openingParenIndex));
    ...
    result = result.slice(0, openingParenIndex + 1) +
             blurChar.repeat(argumentsLength) + result.slice(closingParenIndex);
