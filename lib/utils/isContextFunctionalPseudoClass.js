...
/**
 * Check whether a node is a context-functional pseudo-class (i.e. either a
 * logical combination or a 'aNPlusBOfSNotationPseudoClasses' / tree-structural
 * pseudo-class)
 *
 * @param {import('postcss-selector-parser').Pseudo} node
 *     postcss-selector-parser node (of type pseudo)
...
module.exports = function isContextFunctionalPseudoClass(node) {
  if (node.type === 'pseudo') {
    ...
    return (
        keywordSets.logicalCombinationsPseudoClasses.has(
