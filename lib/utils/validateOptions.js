 ...
 *      options are an object, with corresponding keys; if the options are not
 * an object, `possible` isn't, either. All `possible` value representations
 *      should be **arrays of either values or functions**. Values are ===
 * checked against `actual`. Functions are fed `actual` as an argument and their
...
function validate(opts, ruleName, complain) {
    ...
    if (optional) {
      complain(
          `Incorrect configuration for rule "${
              ruleName}". Rule should have "possible" values for options validation`,
      );
  ...
  Object.keys(actual).forEach((optionName) => {
    ...
    [].concat(actualOptionValue).forEach((a) => {
      ...
      complain(`Invalid value "${a}" for option "${optionName}" of rule "${
          ruleName}"`);
    });
  });
}
