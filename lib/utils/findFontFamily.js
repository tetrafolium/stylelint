...
module.exports = function findFontFamily(value) {
  ...
  valueNodes.walk((valueNode, index, nodes) => {
    ...
    if (keywordSets.fontShorthandKeywords.has(valueLowerCase) &&
    ...
    if (nodes[index - 1] && nodes[index - 1].value === '/' &&
        nodes[index - 2] && isValidFontSize(nodes[index - 2].value)) {
      return;
    }
    ...
    // Detect when a space or comma is dividing a list of font-families, and
    // save the joining character.
    if ((valueNode.type === 'space' ||
    ...
    if (needMergeNodesByValue) {
      joinValueNodes(fontFamilies[fontFamilies.length - 1], valueNode,
                     mergeCharacters);
