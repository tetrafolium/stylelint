 ...
 *   between the `targetWhitespace` (almost definitely a newline) and the
 * `index`. With this option, the checker will see if a newline *begins* the
 * whitespace before the `index`.
 */
 ...
 * @param { "always" | "never" | "always-single-line" | "always-multi-line" |
 *     "never-single-line" | "never-multi-line" } expectation
...
module.exports = function(targetWhitespace, expectation, messages) {
  ...
  */
   ...
   */
  function after(
      {source, index, err, errTarget, lineCheckStr, onlyOneChar = false}) {
  ...
  function expectBefore(messageFunc = messages.expectedBefore) {
    ...
    activeArgs.err(
        msgFunc(activeArgs.errTarget ? activeArgs.errTarget : source[index]));
  }
   ...
   * @param {WhitespaceCheckerArgs} obj
   */
  function afterOneOnly(obj) { after({...obj, onlyOneChar : true}); }
 ...
 * @param {any} x
 */
function isValue(x) { return x !== undefined && x !== null; }
