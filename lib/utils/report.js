...
@typedef {{
  ...
  node: import('postcss').Node & {
    positionBy(opts: { index?: number, word?: string }): { line: number, column:
number }
  },
  index?: number,
  word?: string,
...
module.exports = function(violation) {
  ...
  if (result.stylelint.disabledRanges &&
      (!result.stylelint.ignoreDisables ||
       result.stylelint.reportNeedlessDisables)) {
    const ranges = result.stylelint.disabledRanges[ruleName] ||
  ...
  /** @type {string} */
  const severity =
      _.get(result.stylelint, [ 'ruleSeverities', ruleName ], 'ignore');
  ...
  const warningMessage =
      _.get(result.stylelint, [ 'customMessages', ruleName ], message);
