...
describe('isStandardSyntaxRule', () => {
  ...
  it('compound custom-selectors', () => {
    expect(
        isStandardSyntaxRule(node(':--custom-selector:--custom-selector {}')))
        .toBeTruthy();
  });
  ...
  it('non-outputting Less class mixin definition', () => {
    expect(isStandardSyntaxRule(lessNode('.mixin-name(@a, @b) {}')))
        .toBeFalsy();
  });
  it('non-outputting parametric Less class mixin definition ending in number',
     () => {
       expect(isStandardSyntaxRule(lessNode('.mixin-name3(@a, @b) {}')))
           .toBeFalsy();
     });
  ...
  it('called namespaced Less mixin (child)', () => {
    expect(isStandardSyntaxRule(lessNode('#namespace > .mixin-name;')))
        .toBeFalsy();
  });
  ...
  it('less mixin', () => {
    expect(
        isStandardSyntaxRule(
            lessNode('.box-shadow(@style, @c) when (iscolor(@c)) {}')),
        )
        .toBeFalsy();
  });
  it('less extend', () => {
  ...
  it('less guarded namespaces', () => {
    expect(isStandardSyntaxRule(lessNode('#namespace when (@mode=huge) {}')))
        .toBeFalsy();
  });
  it('mixin guards', () => {
    expect(
        isStandardSyntaxRule(
            lessNode('.mixin (@variable) when (@variable = 10px) {}')),
        )
        .toBeFalsy();
  });
  it('css guards', () => {
    expect(isStandardSyntaxRule(lessNode('.foo() when (@variable = true) {}')))
        .toBeFalsy();
  });
  ...
  it('css guards with multiple spaces', () => {
    expect(
        isStandardSyntaxRule(lessNode('.foo()   when   (@variable = true) {}')))
        .toBeFalsy();
  });
  it('css guards with newlines', () => {
    expect(
        isStandardSyntaxRule(lessNode('.foo()\nwhen\n(@variable = true) {}')))
        .toBeFalsy();
  });
  it('css guards with CRLF', () => {
    expect(isStandardSyntaxRule(
               lessNode('.foo()\r\nwhen\r\n(@variable = true) {}')))
        .toBeFalsy();
  });
  it('css guards with parenthesis', () => {
    expect(isStandardSyntaxRule(lessNode('.foo() when (default()) {}')))
        .toBeFalsy();
  });
  it('css guards with not', () => {
    expect(
        isStandardSyntaxRule(lessNode('.foo() when not (@variable = true) {}')))
        .toBeFalsy();
  });
});
