...
describe('isStandardSyntaxFunction', () => {
  it('calc', () => {
    expect(isStandardSyntaxFunction(func('a { prop: calc(a + b) }')))
        .toBe(true);
  });

  it('url', () => {
    expect(isStandardSyntaxFunction(func("a { prop: url('x.css') }")))
        .toBe(true);
  });
  ...
  it('scss map', () => {
    expect(isStandardSyntaxFunction(func('a { $map: (key: value) }')))
        .toBe(false);
  });
});
