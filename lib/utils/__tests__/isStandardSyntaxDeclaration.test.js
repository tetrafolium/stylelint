...
describe('isStandardSyntaxDeclaration', () => {
  ...
  it('custom-property', () => {
    expect(isStandardSyntaxDeclaration(decl('a { --custom-property: x }')))
        .toBe(true);
  });

  it('standard prop and calc value', () => {
    expect(isStandardSyntaxDeclaration(decl('a { a : calc(b + c) }')))
        .toBe(true);
  });
  ...
  it('custom property set in SCSS parser', () => {
    expect(
        isStandardSyntaxDeclaration(
            scssDecl('a { --custom-property-set: { color: blue; } }')),
        )
        .toBe(true);
  });
  ...
  it('custom property set in LESS parser', () => {
    expect(
        isStandardSyntaxDeclaration(
            lessDecl('a { --custom-property-set: { color: blue; } }')),
        )
        .toBe(true);
  });
  ...
  it('property with scss variable interpolation (only)', () => {
    expect(isStandardSyntaxDeclaration(scssDecl('a { #{$var}: 10px; }')))
        .toBe(true);
  });
  ...
  it('property with scss variable interpolation (end)', () => {
    expect(isStandardSyntaxDeclaration(scssDecl('a { prop#{$var}: 10px; }')))
        .toBe(true);
  });
  ...
  it('property with scss variable interpolation (middle)', () => {
    expect(
        isStandardSyntaxDeclaration(scssDecl('a { prop#{$var}erty: 10px; }')))
        .toBe(true);
  });

  it('property with less variable interpolation (only)', () => {
    expect(isStandardSyntaxDeclaration(lessDecl('a { @{var}: 10px; }')))
        .toBe(true);
  });

  it('property with less variable interpolation (end)', () => {
    expect(isStandardSyntaxDeclaration(lessDecl('a { prop@{var}: 10px; }')))
        .toBe(true);
  });

  it('property with less variable interpolation (middle)', () => {
    expect(isStandardSyntaxDeclaration(lessDecl('a { prop@{var}erty: 10px; }')))
        .toBe(true);
  });
  ...
  it('scss var within namespace', () => {
    expect(isStandardSyntaxDeclaration(scssDecl('namespace.$var: b')))
        .toBe(false);
  });

  it('nested scss var within namespace', () => {
    expect(isStandardSyntaxDeclaration(scssDecl('a { namespace.$var: b }')))
        .toBe(false);
  });

  it('sass list', () => {
    expect(isStandardSyntaxDeclaration(
               sassDecl('$list: (key: value, key2: value2)')))
        .toBe(false);
  });

  it('scss list', () => {
    expect(isStandardSyntaxDeclaration(
               scssDecl('$list: (key: value, key2: value2)')))
        .toBe(false);
  });

  it('sass map', () => {
    expect(isStandardSyntaxDeclaration(sassDecl('$map: (value, value2)')))
        .toBe(false);
  });

  it('scss map', () => {
    expect(isStandardSyntaxDeclaration(scssDecl('$map: (value, value2)')))
        .toBe(false);
  });
  ...
  it('nested scss list', () => {
    expect(isStandardSyntaxDeclaration(
               scssDecl('a { $list: (key: value, key2: value2) }')))
        .toBe(
            false,
        );
  });
  ...
  it('nested scss map', () => {
    expect(isStandardSyntaxDeclaration(scssDecl('a { $map: (value, value2) }')))
        .toBe(false);
  });

  it('less &:extend', () => {
    expect(isStandardSyntaxDeclaration(lessDecl('a { &:extend(b) }')))
        .toBe(false);
  });
});
...
function sassDecl(css) { return decl(css, postcssSass); }

function scssDecl(css) { return decl(css, postcssScss); }

function lessDecl(css) { return decl(css, postcssLess); }
