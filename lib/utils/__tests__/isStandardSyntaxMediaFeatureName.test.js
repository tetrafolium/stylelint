...
const isStandardSyntaxMediaFeatureName =
    require('../isStandardSyntaxMediaFeatureName');
...
describe('isStandardSyntaxMediaFeatureName', () => {
  ...
  it('vendor prefixed keyword', () => {
    expect(isStandardSyntaxMediaFeatureName('-webkit-min-device-pixel-ratio'))
        .toBeTruthy();
  });
  ...
  it('scss var single quoted addition', () => {
    expect(isStandardSyntaxMediaFeatureName("'min-width + $value'"))
        .toBeFalsy();
  });
  it('scss var single quoted added to', () => {
    expect(isStandardSyntaxMediaFeatureName("'$value + min-width'"))
        .toBeFalsy();
  });
