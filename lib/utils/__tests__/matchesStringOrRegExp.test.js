...
it('matchesStringOrRegExp comparing with string comparisonValues', () => {
  ...
  expect(matchesStringOrRegExp('bar something/', 'bar something//'))
      .toBeFalsy();
  ...
  expect(matchesStringOrRegExp([ 'bar', 'hooha' ], [ 'foo', 'baz' ]))
      .toBeFalsy();
});
...
it('matchesStringOrRegExp comparing with a RegExp comparisonValue', () => {
  ...
  expect(matchesStringOrRegExp([ '.foo', 'ebarz' ], [ '/\\.foo$/', '/^bar/' ]))
      .toEqual({
        match : '.foo',
        pattern : '/\\.foo$/',
      });
  expect(matchesStringOrRegExp([ 'bar', 'foo' ], [ '/\\.foo$/', '/^bar/' ]))
      .toEqual({
        match : 'bar',
        pattern : '/^bar/',
      });
  expect(matchesStringOrRegExp([ 'ebarz', 'foo' ], [ '/\\.foo$/', '/^bar/' ]))
      .toBeFalsy();
