...
describe('isStandardSyntaxTypeSelector', () => {
  ...
  it('lowercase nth-child pseudo selector', () => {
    expect(isStandardSyntaxTypeSelector(tag('.foo:nth-child(n) {}')))
        .toBe(false);
  });
  ...
  it('nth-last-child pseudo selector', () => {
    expect(isStandardSyntaxTypeSelector(tag('.foo:nth-last-child(n) {}')))
        .toBe(false);
  });
  it('nth-of-type pseudo selector', () => {
    expect(isStandardSyntaxTypeSelector(tag('.foo:nth-of-type(n) {}')))
        .toBe(false);
  });
  ...
  // eslint-disable-next-line jest/no-disabled-tests -- "Reference combinator"
  // is unsupported? (see #2508)
  it.skip('shadow-piercing descendant combinator', () => {
    expect(isStandardSyntaxTypeSelector(tag('.foo /deep/ .bar {}')))
        .toBe(false);
  });
...
function tag(css) {
  ...
  postcss.parse(css).walkRules((rule) => {
    selectorParser(
        (selectorAST) => { selectorAST.walkTags((t) => tags.push(t)); })
