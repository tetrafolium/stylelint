...
describe('isStandardSyntaxAtRule', () => {
  ...
  it('non nested at-rules with function and without space after name', () => {
    expect(
        isStandardSyntaxAtRule(atRule('@import url("fineprint.css") print;')))
        .toBeTruthy();
  });

  it('nested at-rules', () => {
    expect(isStandardSyntaxAtRule(atRule('@media (min-width: 100px) {};')))
        .toBeTruthy();
  });
  ...
  // eslint-disable-next-line jest/no-disabled-tests -- TODO: `postcss-sass`
  // parser does not support `@mixin`.
  ...
  it('ignore variables', () => {
    const rules =
        lessAtRules('@my-variable: 10px; .top { margin-top: @my-variable; }');
...
function atRule(code) { return atRules(code)[0]; }

function sassAtRules(code) { return atRules(code, postcssSass); }

function scssAtRules(code) { return atRules(code, postcssScss); }

function lessAtRules(code) { return atRules(code, postcssLess); }
