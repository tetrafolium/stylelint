...
describe('isLessVariable', () => {
  ...
  it('is less variable with function', () => {
    expect(isLessVariable(lessAtRule('@hover-color: darken(@color, 10%);')))
        .toBeTruthy();
  });
  ...
  it('@import is not a less variable', () => {
    expect(isLessVariable(lessAtRule('@import url("some-styles.css");')))
        .toBeFalsy();
  });

  it('@media is not a less variable', () => {
    expect(isLessVariable(lessAtRule('@media (min-width: 100px) {};')))
        .toBeFalsy();
  });

  it('detached ruleset is not a less variable', () => {
    expect(isLessVariable(lessAtRule('@detached-ruleset: { margin: 0 };')))
        .toBeFalsy();
  });
});
