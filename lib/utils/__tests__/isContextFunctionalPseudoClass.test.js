...
const isContextFunctionalPseudoClass =
    require('../isContextFunctionalPseudoClass');
...
function pseudo(css) {
  ...
  postcss.parse(css).walkRules((rule) => {
    parseSelector(
        (selectorAST) => { selectorAST.walkPseudos((p) => pseudos.push(p)); })
...
describe('isContextFunctionalPseudoClass', () => {
  ...
  it('handles logical combinations', () => {
    ...
    expect(isContextFunctionalPseudoClass(pseudo('a:matches(.foo) {}')))
        .toBe(true);
    expect(isContextFunctionalPseudoClass(pseudo('a:not(.foo) {}'))).toBe(true);
    expect(isContextFunctionalPseudoClass(pseudo('a:where(.foo) {}')))
        .toBe(true);
  });

  it('handles tree structural/NPlusBOfSNotationPseudoClasses classes', () => {
    expect(isContextFunctionalPseudoClass(pseudo('a:nth-child(n+7) {}')))
        .toBe(true);
    expect(isContextFunctionalPseudoClass(pseudo('a:nth-last-child(n+7) {}')))
        .toBe(true);
  });
});
