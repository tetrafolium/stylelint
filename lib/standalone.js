...
/**
 * @typedef {import('stylelint').StylelintStandaloneOptions}
 * StylelintStandaloneOptions
 */
/**
 * @typedef {import('stylelint').StylelintStandaloneReturnValue}
...
module.exports = function(options) {
  ...
  if (!files) {
    ...
    if (absoluteCodeFilename &&
      ...
      return Promise.resolve(
          prepareReturnValue([], options, formatterFunction));
    }
    ...
    return stylelint
        ...
        .then((stylelintResult) => {
          ...
          const returnValue = prepareReturnValue([ stylelintResult ], options,
                                                 formatterFunction);

          if (options.fix && postcssResult &&
            ...
            if (!postcssResult.stylelint.disableWritingFix) {
              ...
              // If the writing of the fix is disabled, the input code is
              // returned as-is
  ...
  fileList = fileList.map((entry) => {
    ...
    const absolutePath =
        !path.isAbsolute(entry) ? path.join(cwd, entry) : path.normalize(entry);
  ...
  if (useCache) {
    ...
    const hashOfConfig =
        hash(`${stylelintVersion}_${JSON.stringify(config || {})}`);
  ...
  return globby(fileList, globbyOptions)
        ...
        const getStylelintResults = absoluteFilePaths.map((absoluteFilepath) => {
          ...
          return stylelint
              ...
              .then((postcssResult) => {
                ...
                if (postcssResult.root && postcssResult.opts &&
                    !postcssResult.stylelint.ignored && options.fix &&
                    !postcssResult.stylelint.disableWritingFix) {
                  // @ts-ignore TODO TYPES toString accepts 0 arguments
                  const fixedCss =
                      postcssResult.root.toString(postcssResult.opts.syntax);

                  if (postcssResult.root && postcssResult.root.source &&
                ...
                return fixFile.then(
                    () => stylelint._createStylelintResult(postcssResult,
                                                           absoluteFilepath),
                );
              })
              .catch((error) => {
      ...
      .then((stylelintResults) => {
        ...
        const rtn =
            prepareReturnValue(stylelintResults, options, formatterFunction);
...
function getFormatterFunction(selected) {
  ...
  if (typeof selected === 'string') {
    ...
    if (formatterFunction === undefined) {
      throw new Error(
          `You must use a valid formatter option: ${
              getFormatterOptionsText()} or a function`,
      );
    }
