...
function invalidOptionsFormatter(results) {
  const allInvalidOptionWarnings = _.flatMap(
      results,
      (r) => r.invalidOptionWarnings.map((w) => w.text),
  );
...
function getMessageWidth(columnWidths) {
  ...
  // If there is no reason to wrap the text, we won't align the last column to
  // the right
...
function formatter(messages, source) {
  ...
  function calculateWidths(columns) {
    for (const [key, value] of Object.entries(columns)) {
      ...
      columnWidths[key] =
          Math.max(columnWidths[key], stringWidth(normalisedValue));
    }
  ...
  const cleanedMessages = orderedMessages.map((message) => {
    ...
    const severity =
        /** @type {keyof import('log-symbols')} */ (message.severity);
    /**
    ...
    const row = [
      ...
      symbols[severity] ? chalk[/** @type {'blue' | 'red' | 'yellow'} */ (
                              levelColors[severity])](symbols[severity])
                        : severity,
      message
          .text
