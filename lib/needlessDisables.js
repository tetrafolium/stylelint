...
module.exports = function(results) {
  results.forEach((result) => {
    ...
    const rangeData =
        _.cloneDeep(result._postcssResult.stylelint.disabledRanges);
    ...
    const disabledWarnings =
        result._postcssResult.stylelint.disabledWarnings || [];
      ...
      if (ruleRanges) {
        for (const range of ruleRanges) {
          if (isWarningInRange(warning, range)) {
            putIfAbsent(usefulDisables, range.comment, () => new Set())
                .add(rule);
          }
        }
      }
    ...
    for (const [rule, ranges] of rangeEntries) {
      for (const range of ranges) {
        ...
        // Only emit a warning if this range's comment isn't useful for this
        // rule. For the special rule "all", only emit a warning if it's not
        // useful for *any* // rules, becuase it covers all of them.
        if (rule === 'all' ? useful.size !== 0 : useful.has(rule))
          continue;

        // If the comment doesn't have a location, we can't report a useful
        // error. In practice we expect all comments to have locations, though.
        if (!range.comment.source || !range.comment.source.start)
          continue;
...
function isWarningInRange(warning, range) {
  ...
  // Need to check if range.end exist, because line number type cannot be
