 ...
 */
/**
 * @typedef {import('stylelint').StylelintConfigProcessor}
 * StylelintConfigProcessor
 */
/**
 * @typedef {import('stylelint').StylelintConfigProcessors}
 * StylelintConfigProcessors
 */
...
function augmentConfigFull(stylelint, cosmiconfigResult) {
  ...
  return augmentConfigBasic(stylelint, config, configDir, true)
      .then(
          (augmentedConfig) => { return addPluginFunctions(augmentedConfig); })
      .then((augmentedConfig) => {
...
function absolutizePaths(config, configDir) {
  if (config.ignoreFiles) {
    config.ignoreFiles =
        /** @type {string[]} */ ([]).concat(config.ignoreFiles).map((glob) => {
  ...
  if (config.plugins) {
    config.plugins =
        /** @type {string[]} */ ([])
            ...
            .map((lookup) => { return getModulePath(configDir, lookup); });
  }
...
function absolutizeProcessors(processors, configDir) {
  const normalizedProcessors =
      Array.isArray(processors) ? processors : [ processors ];
...
function extendConfig(stylelint, config, configDir) {
  ...
  const normalizedExtends = Array.isArray(config.extends) ? config.extends
      : [ config.extends ];
  const {extends : configExtends, ...originalWithoutExtends} = config;

  const loadExtends =
      normalizedExtends.reduce((resultPromise, extendLookup) => {
        return resultPromise.then((resultConfig) => {
          return loadExtendedConfig(stylelint, resultConfig, configDir,
                                    extendLookup)
              .then(
                  (extendResult) => {
                    if (!extendResult)
...
function mergeConfigs(a, b) {
  ...
  if (a.processors || b.processors) {
    ...
    if (a.processors) {
      processorMerger.processors =
          processorMerger.processors.concat(a.processors);
    }
  ...
  const result =
      {...a, ...b, ...processorMerger, ...pluginMerger, ...rulesMerger};
      ...
      if (!pluginRuleDefinition.ruleName) {
        throw configurationError(
            'stylelint v3+ requires plugins to expose a ruleName. ' +
                `The plugin "${
                    pluginLookup}" is not doing this, so will not work ` +
            ...
            'stylelint v7+ requires plugin rules to be namespaced, ' +
                ...
                `The plugin rule "${
                    pluginRuleDefinition
                        .ruleName}" does not do this, so will not work. ` +
  ...
  /** @type {Array<StylelintConfigProcessor>} */ ([])
      ...
      .forEach((processorConfig) => {
        ...
        if (processorCache.has(processorKey)) {
          ...
          const processorLookup = typeof processorConfig === 'string'
                                      ? processorConfig
                                      : processorConfig[0];
          const processorOptions = typeof processorConfig === 'string'
                                       ? undefined
                                       : processorConfig[1];
