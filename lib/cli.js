...
const disableOptionsReportStringFormatter =
    require('./formatters/disableOptionsReportStringFormatter');
  ...
  help : `
    ...
    Options:
      ...
      --formatter, -f               [default: "string"]
        ...
        The output formatter: ${getFormatterOptionsText({
    useOr : true
  })}.
      ...
      EXIT_CODE_ERROR} if needless disables are found.

      --report-invalid-scope-disables, --risd

        Report stylelint-disable comments that used for rules that don't exist within the configuration object.
        The process will exit with code ${
      EXIT_CODE_ERROR} if invalid scope disables are found.

      --report-descriptionless-disables, --rdd

        Report stylelint-disable comments without a description.
        The process will exit with code ${
      EXIT_CODE_ERROR} if descriptionless disables are found.

      --max-warnings, --mw

        Number of warnings above which the process will exit with code ${
      EXIT_CODE_ERROR}.
...
module.exports = (argv) => {
  ...
  const invalidOptionsMessage =
      checkInvalidCLIOptions(meowOptions.flags, cli.flags);
  ...
  if (cli.flags.customFormatter) {
    const customFormatter =
        path.isAbsolute(cli.flags.customFormatter)
  ...
  if (cli.flags.customSyntax) {
    optionsBase.customSyntax =
        getModulePath(process.cwd(), cli.flags.customSyntax);
  }
  ...
  if (cli.flags.configBasedir) {
    optionsBase.configBasedir =
        path.isAbsolute(cli.flags.configBasedir)
  ...
  return Promise.resolve()
      .then(
          ...
          () => {
            ...
            if (cli.input.length) {
              return Promise.resolve(
                  {...optionsBase, files : /** @type {string} */ (cli.input)});
            }
        ...
        return standalone(options)
            .then((linted) => {
              ...
              if (options.outputFile) {
                writeOutputFile(linted.output, options.outputFile)
                    .catch(handleError);
              }
