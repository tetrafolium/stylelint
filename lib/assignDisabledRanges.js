 ...
 */
function createDisableRange(comment, start, strictStart, description, end,
                            strictEnd) {
  return {
    comment,
    start,
...
module.exports = function(root, result) {
  ...
  root.walkComments((/** @type {PostcssComment} */ comment) => {
    ...
    if (!(isInlineComment(comment) && isStylelintCommand(comment) && next &&
          next.type === 'comment' &&
    ...
    let lastLine =
        (comment.source && comment.source.end && comment.source.end.line) || 0;
    ...
    while (isInlineComment(current) && !isStylelintCommand(current)) {
      const currentLine =
          (current.source && current.source.end && current.source.end.line) ||
          0;
  ...
  function isStylelintCommand(comment) {
    return comment.text.startsWith(disableCommand) ||
           comment.text.startsWith(enableCommand);
  }
  ...
  function processDisableNextLineCommand(comment) {
    if (comment.source && comment.source.end) {
      ...
      getCommandRules(disableNextLineCommand, comment.text)
          .forEach((ruleName) => {
  ...
  function disableLine(comment, line, ruleName, description) {
    ...
    if (ruleName === ALL_RULES) {
      Object.keys(disabledRanges).forEach((disabledRuleName) => {
        if (ruleIsDisabled(disabledRuleName))
          return;

        const strict = disabledRuleName === ALL_RULES;

        startDisabledRange(comment, line, disabledRuleName, strict,
                           description);
  ...
  function processDisableCommand(comment) {
    ...
    getCommandRules(disableCommand, comment.text).forEach((ruleToDisable) => {
      ...
      if (ruleIsDisabled(ruleToDisable)) {
        throw comment.error(
            isAllRules ? 'All rules have already been disabled'
      ...
      if (comment.source && comment.source.start) {
        ...
        if (isAllRules) {
          Object.keys(disabledRanges).forEach((ruleName) => {
            startDisabledRange(comment, line, ruleName, ruleName === ALL_RULES,
                               description);
          });
        } else {
  ...
  function processEnableCommand(comment) {
    getCommandRules(enableCommand, comment.text).forEach((ruleToEnable) => {
      ...
      if (ruleToEnable === ALL_RULES) {
        if (Object.values(disabledRanges)
                .every(
                    (ranges) =>
                        ranges.length === 0 ||
                        typeof ranges[ranges.length - 1].end === 'number',
                    )) {
      ...
      if (ruleIsDisabled(ALL_RULES) &&
        ...
        if (!disabledRanges[ruleToEnable]) {
          disabledRanges[ruleToEnable] = disabledRanges.all.map(
              ({start, end, description}) => createDisableRange(
                  comment, start, false, description, end, false),
          );
        } else {
  ...
  function getCommandRules(command, fullText) {
    const rules =
        fullText.slice(command.length)
            .split(
                /\s-{2,}\s/u)[0] // Allow for description (f.e. /*
                                 // stylelint-disable a, b -- Description */).
            .trim()
            .split(',')
            .filter(Boolean)
            .map((r) => r.trim());
  ...
  function ensureRuleRanges(ruleName) {
    if (!disabledRanges[ruleName]) {
      disabledRanges[ruleName] = disabledRanges.all.map(
          ({comment, start, end, description}) => createDisableRange(
              comment, start, false, description, end, false),
      );
    }
  }
  ...
  function ruleIsDisabled(ruleName) {
    if (disabledRanges[ruleName] === undefined)
      return false;

    if (_.last(disabledRanges[ruleName]) === undefined)
      return false;
