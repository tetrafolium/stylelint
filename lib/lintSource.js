...
module.exports = function lintSource(stylelint, options = {}) {
  if (!options.filePath && options.code === undefined &&
  ...
  return getIsIgnored.then((isIgnored) => {
    ...
    const getConfig =
        stylelint.getConfigForFile(configSearchPath).catch((err) => {
          if (isCodeNotFile && err.code === 'ENOENT')
    ...
    return getConfig.then((result) => {
      ...
      if (existingPostcssResult) {
        ...
        return lintPostcssResult(stylelint._options, stylelintPostcssResult,
                                 config)
            .then(
