...
async function caseConfig(caseNumber) {
  return JSON.parse(
      await fs.readFile(caseConfigFile(caseNumber, 'json'), 'utf8'));
}
...
async function caseFilesForFix(caseNumber, ext = 'css') {
  const tempPath = replaceBackslashes(
      path.join(os.tmpdir(), `stylesheet-${_.uniqueId()}.${ext}`));

  await fs.copyFile(path.join(__dirname, caseNumber, `stylesheet.${ext}`),
                    tempPath);
...
function prepForSnapshot({results, output, ...rest}) {
  ...
  if (output.startsWith('[')) {
    // The `source` of each file varies between platforms or if a tmp file is
    // used
